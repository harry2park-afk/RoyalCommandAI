(()=>{
  ...
  try{
    ...
    let p={};
    try{p=JSON.parse(localStorage.getItem(PK)||'{}')}catch{}
    if(!Array.isArray(p.rightPanelApps)||!p.rightPanelApps.length){
      p.rightPanelApps=APPS;
  ...
  const active=x=>{
    if(x){sessionStorage.setItem(AK,x);localStorage.setItem('rca-active',x);return x}
    return sessionStorage.getItem(AK)||localStorage.getItem('rca-active')||'';
  };
  const list=()=>{
    try{const a=JSON.parse(localStorage.getItem(K)||'[]');if(a.length)return a}catch{}
    const a=[fresh()];save(a);active(a[0].id);return a;
  };
  const current=()=>{
    const a=list();
    let c=a.find(x=>x.id===active()&&x.status!=='archived')||a.find(x=>x.status!=='archived');
    if(!c){c=fresh();a.unshift(c);save(a)}
    active(c.id);return c;
  };
  ...
  async function session(){
    try{
      let r=await F('/api/au-v2/session',{cache:'no-store'}),d=await r.json().catch(()=>({}));
  ...
  function publishLastRun(prompt,language,responses){
    const run={
    ...
    try{sessionStorage.setItem(LAST,JSON.stringify(run))}catch{}
    try{window.dispatchEvent(new CustomEvent('rca:last-run',{detail:run}))}catch{}
  }

  function readLastRun(){
    try{return JSON.parse(sessionStorage.getItem(LAST)||'null')}catch{return null}
  }

  function persist(prompt,res){
    const a=list(),c=current(),t=a.find(x=>x.id===c.id)||c,n=iso();
    t.messages.push({id:id('u'),content:prompt,author_type:'user'});
    res.forEach(x=>{
      const name=N[x.provider]||x.provider;
      const txt=x.error&&!String(x.content||'').trim()
        ?`### ${name}\n⚠️ ${x.error}`
        :`### ${name}\n${String(x.content||'').trim()}${x.error?`\n\n⚠️ ${x.error}`:''}`;
      t.messages.push({id:id('a'),content:txt,author_type:'ai'});
    });
    if(t.title==='New Chat')t.title=prompt.replace(/\s+/g,' ').slice(0,72);
    t.updated_at=t.last_message_at=n;
    save([t,...a.filter(x=>x.id!==t.id)]);
  }

  function synthsave(txt){
    if(!txt)return;
    const a=list(),c=current(),t=a.find(x=>x.id===c.id)||c,n=iso();
    t.messages.push({id:id('s'),content:`### 통합 답변\n${txt}`,author_type:'ai'});
    t.updated_at=t.last_message_at=n;
    save([t,...a.filter(x=>x.id!==t.id)]);
  }
  ...
  window.fetch=async(i,o)=>{
    ...
    if(p==='/api/rooms/rca/conversations'&&m==='POST'){
      const b=await body(i,o),c=fresh();
      if(typeof b.title==='string')c.title=b.title.slice(0,120);
      const a=list();a.unshift(c);save(a);active(c.id);clearLastRun();
      return J({conversation:meta(c)},201);
    }

    const q=p.match(/^\/api\/rooms\/rca\/conversations\/([^/]+)$/);
    if(q){
      const a=list(),c=a.find(x=>x.id===q[1]);
      if(!c)return J({error:'Conversation not found'},404);
      if(m==='GET')return J({conversation:meta(c),messages:c.messages||[]});
      if(m==='PATCH'){
        const b=await body(i,o);
        if(typeof b.title==='string'&&b.title.trim())c.title=b.title.trim().slice(0,120);
        if(b.status==='active'||b.status==='archived')c.status=b.status;
        c.updated_at=iso();save(a);
        if(c.status==='active')active(c.id);
        return J({conversation:meta(c)});
      }
    }

    if(p==='/api/user/preferences'){
      if(m==='GET'){
        let x={};try{x=JSON.parse(localStorage.getItem(PK)||'{}')}catch{}
        if(!Array.isArray(x.rightPanelApps)||!x.rightPanelApps.length)x.rightPanelApps=APPS;
        return J({preferences:x});
      }
      if(m==='PATCH'){
        const b=await body(i,o);let x={};try{x=JSON.parse(localStorage.getItem(PK)||'{}')}catch{}
        x={...x,...b};localStorage.setItem(PK,JSON.stringify(x));return J({preferences:x});
      }
    }

    if(p==='/api/ai/chat/stream'&&m==='POST'){
      const b=await body(i,o);
      if(!await session())return J({error:'RCA session failed'},401);
      const r=await F('/api/au-v2/chat',{
        method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({prompt:b.prompt,providers:b.providers,history:b.history})
      });
      const d=await r.json().catch(()=>({}));
      if(!r.ok)return J({error:d.error||'AI request failed'},r.status);
      const res=Array.isArray(d.responses)?d.responses:[];
      const prompt=String(b.prompt||'');
      persist(prompt,res);
      publishLastRun(prompt,b.language,res);
    ...
    if(p==='/api/ai/synthesize'&&m==='POST'){
      ...
      const r=await F('/api/au-v2/synthesize',{
        method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({originalPrompt,synthesizer:b.synthesizer,responses})
      });
      const d=await r.json().catch(()=>({}));
      if(r.ok&&d.finalAnswer)synthsave(String(d.finalAnswer));
      return J(d,r.status);
    }
