(() => {
  ...
  const ORDER_KEY = "royalcommand:language-order";
  ...
  function loadOrder() {
    try {
      ...
      const missing = LANGS.map((x) => x[0]).filter((v) => !valid.includes(v));
      return [...valid, ...missing];
    } catch {
      return LANGS.map((x) => x[0]);
    }
  }
  ...
  function pinFirst(order, value) {
    return [value, ...order.filter((v) => v !== value)];
  }
  ...
  function enhance(select) {
    ...
    hint.textContent = "Drag ↕ to reorder. Selected language stays first.";
    ...
    let order = pinFirst(loadOrder(), current[0]);
    saveOrder(order);
    ...
    function moveItem(dragValue, targetValue) {
      if (!dragValue || !targetValue || dragValue === targetValue || dragValue === current[0]) return;
    ...
    function renderList(query = "") {
      ...
      items.forEach((x, index) => {
        ...
        row.innerHTML = `<span style="width:16px;color:${x[0] === current[0] ? "var(--gold-soft)" : "#708090"};font-size:13px">${x[0] === current[0] ? "★" : "↕"}</span><img src="${flagUrl(x[2])}" alt="" width="24" height="16" style="width:24px;height:16px;object-fit:cover;border-radius:2px"><strong style="width:30px;color:var(--gold-soft)">${x[1]}</strong><span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1">${x[3]}</span>${index === 0 ? '<span style="font-size:9px;color:var(--gold-soft)">TOP</span>' : ''}`;
        ...
        row.ondragover = (e) => { if (!q && x[0] !== current[0]) { e.preventDefault(); e.dataTransfer.dropEffect = "move"; } };
        row.ondrop = (e) => {
          if (q || x[0] === current[0]) return;
          e.preventDefault();
    ...
    button.onclick = (e) => {
      ...
      if (opening) {
        order = pinFirst(loadOrder(), current[0]);
        saveOrder(order);
        positionMenu();
        search.value = "";
        renderList();
