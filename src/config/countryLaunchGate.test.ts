...
describe("country launch readiness gate", () => {
  it("keeps the first wave plus next-priority Singapore, China and Hong Kong blocked until launch-critical reviews and connections are verified", () => {
  ...
  it("keeps Singapore, China and Hong Kong explicitly blocked before human review and provider connection", () => {
    for (const countryCode of ["SG", "CN", "HK"] as const) {
      const config = getCountryConfigByCountryCode(countryCode);
      expect(config, countryCode).not.toBeNull();
