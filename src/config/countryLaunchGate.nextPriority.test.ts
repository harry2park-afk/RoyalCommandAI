...
describe("next-priority country operational launch gate", () => {
  it("applies every first-wave operational safety gate to SG/CN/HK/TW/IN before expansion", () => {
  ...
  it("rejects first-wave evidence reused for a next-priority country", () => {
    for (const countryCode of NEXT_PRIORITY) {
      const base = getCountryConfigByCountryCode(countryCode);
      expect(base, countryCode).not.toBeNull();
      expect(evaluateCountryOperationalLaunch(asConfigReady(base!), readyOperationalEvidence("AU"), RELEASE_SCOPE), countryCode).toEqual({
        launchable: false,
  ...
  it("does not confuse configured next-priority countries with current launch approval", () => {
    for (const countryCode of NEXT_PRIORITY) {
      const base = getCountryConfigByCountryCode(countryCode);
      expect(base, countryCode).not.toBeNull();
      expect(base?.payments.status, countryCode).toBe("NOT_CONNECTED");
      expect(base?.tax.status, countryCode).toBe("NOT_CONNECTED");
      expect(base?.taxStructure?.status, countryCode).toBe("NEEDS_REVIEW");
      expect(Object.values(base?.compliance ?? {}), countryCode).toEqual([
        "NEEDS_REVIEW",
        "NEEDS_REVIEW",
        "NEEDS_REVIEW",
        "NEEDS_REVIEW",
