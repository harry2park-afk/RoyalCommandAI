...
describe("globalLocaleCore", () => {
  ...
  it("maps current Korean and Japanese legacy values safely", () => {
    expect(normalizeLegacyLanguage("ko", "AU")).toBe("ko-KR");
    expect(normalizeLegacyLanguage("ja", "US")).toBe("ja-JP");
  });
  ...
  it("uses all six first-wave country defaults when no language has been selected", () => {
