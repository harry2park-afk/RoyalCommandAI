...
const DEMO_STYLES: Record<AIProviderId, string> = {
...
export class DemoConnector implements AIConnector {
  ...
  async complete(request: AIRequest): Promise<AIProviderResponse> {
    ...
    const style = DEMO_STYLES[this.id];

    return {
      provider: this.id,
      model: `demo-${this.id}`,
      content: `[${this.displayName} · demo]\nA ${style} response to: "${prompt.slice(0, 180)}"\n\nKey points:\n1. Clarify the goal inside your Royal Household Room.\n2. Use connected services through secure ports.\n3. Keep originals immutable and approve important outputs before sending.`,
