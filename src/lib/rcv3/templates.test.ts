...
describe('published room artwork',()=>{
 it('has thirty-nine distinct local assets with no executable or remote URLs',()=>{
  expect(roomTemplates).toHaveLength(39);
  expect(new Set(roomTemplates.map(t=>t.id)).size).toBe(39);
