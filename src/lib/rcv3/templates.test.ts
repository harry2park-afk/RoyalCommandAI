...
describe('published room artwork',()=>{
 it('has thirty-three distinct local assets with no executable or remote URLs',()=>{
  expect(roomTemplates).toHaveLength(33);
  expect(new Set(roomTemplates.map(t=>t.id)).size).toBe(33);
