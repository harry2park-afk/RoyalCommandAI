...
function assistantHistory(turn: Turn) {
  ...
  return turn.answers
    .filter((answer) => !answer.error && answer.content.trim())
    .map((answer) => `${CATALOG_BY_ID[answer.provider]?.shortName || answer.provider}: ${answer.content.trim()}`)
    .join("\n\n");
}
...
export default function RCARoom() {
  ...
  const latestGoodAnswers = latestTurn?.answers.filter((answer) => !answer.error && answer.content.trim()) || [];

  const filteredWarehouse = useMemo(() => {
    const query = warehouseSearch.trim().toLowerCase();
    return AI_CATALOG.filter((ai) => !query || `${ai.name} ${ai.shortName}`.toLowerCase().includes(query));
  }, [warehouseSearch]);

  function isAvailable(id: string) {
    return providers.some((provider) => provider.id === id && provider.available);
  }
  ...
  }, [activeId, conversations, memoryReady]);

  useEffect(() => {
    if (!memoryReady) return;
    localStorage.setItem(SELECTED_KEY, JSON.stringify(selected));
  }, [memoryReady, selected]);
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        const available = AI_CATALOG.map((ai) => ai.id).filter((id) => list.some((provider) => provider.id === id && provider.available));
        let initialSelected = available.includes("openai") ? ["openai"] : available.slice(0, 1);
        try {
          const saved = JSON.parse(localStorage.getItem(SELECTED_KEY) || "[]") as string[];
          const valid = saved.filter((id) => available.includes(id));
          if (valid.length) initialSelected = valid;
        } catch {}
        ...
        setError(cause instanceof Error ? cause.message : "RCA Room could not start.");
      }
    })();
  }, []);
  ...
  }, [activeConversation?.turns.length, busy]);

  function toggleProvider(id: string) {
    if (!isAvailable(id)) {
      setError(`${CATALOG_BY_ID[id]?.shortName || id} is not connected.`);
      return;
    }
    setError("");
  ...
  function replaceWarehouseAI(id: string) {
    setSlots((current) => {
      ...
      const oldId = current[replaceSlot];
      const next = [...current];
      next[replaceSlot] = id;
      setSelected((selectedIds) => selectedIds.filter((selectedId) => selectedId !== oldId));
      return next;
    });
  ...
  async function send(event?: FormEvent) {
    ...
    setConversations((current) => current.map((item) => item.id === conversation.id
      ? {
          ...item,
          title: item.turns.length ? item.title : compactTitle(text),
    ...
    try {
      const response = await fetch("/api/au-v2/chat", {
        ...
        body: JSON.stringify({ prompt: text, providers: activeProviders, history }),
      });
      const data = await response.json().catch(() => ({}));
      if (response.status === 401) {
        setSessionReady(false);
        throw new Error("RCA Room session ended. Refresh the page to restart it.");
      }
      ...
      setConversations((current) => current.map((item) => item.id === conversation.id
        ? {
            ...item,
            turns: item.turns.map((turn) => turn.id === turnId ? { ...turn, answers } : turn),
            updatedAt: Date.now(),
          }
        : item));
    } catch (cause) {
  ...
  function handleSynthesisButton() {
    ...
    if (!sessionReady) {
      setError("RCA Room session is not ready yet.");
      return;
    }
    if (!latestTurn || latestGoodAnswers.length < 2) {
      setError("통합 답변은 최근 질문에 AI 답변이 2개 이상 있어야 합니다.");
      return;
    }
    setError("");
    setSynthOpen((current) => !current);
  }
  ...
  async function synthesize(providerId: string) {
    ...
    try {
      const response = await fetch("/api/au-v2/synthesize", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          originalPrompt: latestTurn.prompt,
          synthesizer: providerId,
          responses: latestGoodAnswers,
        }),
      });
      const data = await response.json().catch(() => ({}));
      ...
      setConversations((current) => current.map((item) => item.id === conversationId
        ? {
            ...item,
            turns: item.turns.map((turn) => turn.id === turnId
              ? { ...turn, synthesis: finalAnswer, synthesizer: providerId }
              : turn),
            updatedAt: Date.now(),
          }
        : item));
    ...
    <main className="flex h-[100dvh] min-h-0 w-full overflow-hidden bg-[#07101d] pt-[92px] text-[#f4f0e7]">
      <div className="fixed inset-x-0 top-0 z-[170] h-[92px] border-b border-[#d7b64d]/30 bg-[#07101d]/98 shadow-lg backdrop-blur">
        <div className="flex h-[42px] items-center gap-2 border-b border-white/10 px-3">
          <Link href="/" className="shrink-0 text-sm text-[#d7b64d]">← Australia</Link>
          <h1 className="ml-8 shrink-0 text-xl font-semibold leading-none">RCA Room</h1>
          <div className="min-w-0 flex-1 text-center text-sm font-semibold text-[#f4f0e7]">Royal Command Australia</div>
          <div className={`shrink-0 rounded-full border px-2 py-1 text-[10px] ${sessionReady ? "border-emerald-400/40 text-emerald-300" : "border-amber-400/40 text-amber-300"}`}>
            {sessionReady ? "Memory + AI Ready" : "Starting…"}
          </div>
        </div>
        ...
        <div className="flex h-[50px] w-full items-center gap-1 overflow-hidden px-2 py-1.5">
            ...
            return (
              ...
              >
                <span className="relative grid h-5 w-5 shrink-0 place-items-center rounded bg-black/20">
                  {AI_LOGOS[id] ? <img src={AI_LOGOS[id]} alt="" className="h-4 w-4 object-contain" /> : null}
          ...
          <button
            type="button"
            onClick={() => { setSynthOpen(false); setWarehouseOpen(true); }}
          ...
          <div className="relative shrink-0">
            <button
              type="button"
              onClick={handleSynthesisButton}
              className={`h-8 min-w-[106px] rounded-md border-[2px] px-3 text-[10px] font-semibold ${latestGoodAnswers.length >= 2 && !busy ? "border-[#FFD700] bg-[#7A0C2E] text-[#FFF3D6]" : "border-white/15 bg-[#293342] text-white/40"}`}
            >
              {synthBusy ? "통합 중…" : "통합 답변 ▾"}
            </button>
            {synthOpen && (
              <div className="absolute right-0 top-10 z-[220] max-h-72 w-52 overflow-y-auto rounded-xl border border-[#d7b64d]/50 bg-[#081321] p-2 shadow-2xl">
                {providers.filter((provider) => provider.available).map((provider) => (
                  <button key={provider.id} type="button" onClick={() => void synthesize(provider.id)} className="block w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-white/10">
                    {provider.name}
                  </button>
                ))}
              </div>
      ...
      <aside className="hidden h-full w-[245px] shrink-0 flex-col border-r border-[#d7b64d]/25 bg-[#081321] md:flex">
        <div className="border-b border-white/10 p-3">
          <button type="button" onClick={startNewChat} className="flex h-10 w-full items-center justify-center gap-2 rounded-lg border border-[#d7b64d]/50 bg-[#102033] text-sm font-semibold text-[#f4d66c]">
            <MessageSquarePlus size={16} /> New Chat
          </button>
        </div>
        <div className="min-h-0 flex-1 overflow-y-auto p-2">
          <div className="mb-2 px-2 text-[11px] uppercase tracking-wide text-white/40">Conversations</div>
          <div className="space-y-1">
            {conversations.map((conversation) => (
              <button
                key={conversation.id}
                type="button"
                onClick={() => { setActiveId(conversation.id); setError(""); setSynthOpen(false); }}
                className={`w-full rounded-lg border px-3 py-2 text-left ${conversation.id === activeId ? "border-[#d7b64d]/60 bg-[#14224D]" : "border-transparent hover:bg-white/5"}`}
              >
                <span className="block truncate text-xs font-semibold text-[#E8E6DD]">{conversation.title}</span>
                <span className="mt-1 block text-[10px] text-white/35">{conversation.turns.length} turn{conversation.turns.length === 1 ? "" : "s"}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="min-h-0 min-w-0 flex-1 overflow-hidden">
        <section className="flex h-full min-h-0 min-w-0 flex-col overflow-hidden border-y border-white/10 bg-[#0B1524]">
          <div ref={messagesViewportRef} className="min-h-0 min-w-0 flex-1 space-y-3 overflow-y-auto overscroll-contain px-3 py-3">
            {!activeConversation?.turns.length && !busy && (
              <div className="mx-auto mt-10 max-w-xl text-center">
                <Bot className="mx-auto text-[#d7b64d]" size={30} />
                <h2 className="mt-2 text-xl font-semibold">RCA Room</h2>
                <p className="mt-2 text-sm text-white/50">기존 RC Room 모양을 유지하고, 내부는 Australia 전용 AI·통합·메모리로 분리했습니다.</p>
              </div>
            )}

            {activeConversation?.turns.map((turn, turnIndex) => (
              <div key={turn.id} className="space-y-2">
                <div className="flex min-h-[33px] w-full items-center rounded-[7px] border-[3px] border-[#FFD700] bg-[#1E3A8A] px-[9px] text-white">
                  <span className="mr-2 shrink-0 text-[10px] text-[#f4d66c]">Q{turnIndex + 1}</span>
                  <span className="min-w-0 flex-1 text-[12px] leading-5">{turn.prompt}</span>
                </div>

                {turn.answers.map((answer) => (
                  <article key={`${turn.id}-${answer.provider}`} className="w-full rounded-[7px] border-2 border-[#2A3B6E] bg-[#14224D] px-4 py-3 text-[#E8E6DD]">
                    <div className="mb-2 text-sm font-semibold text-[#f4d66c]">{CATALOG_BY_ID[answer.provider]?.shortName || answer.provider}</div>
                    <div className="whitespace-pre-wrap text-sm leading-6">{answer.error ? `⚠️ ${answer.error}` : answer.content}</div>
                  </article>
                ))}

                {turn.synthesis && (
                  <article ref={turn.id === latestTurn?.id ? synthesisRef : undefined} className="w-full rounded-[7px] border-[3px] border-[#FFD700] bg-[#132019] px-4 py-4 text-[#E8E6DD]">
                    <div className="mb-2 flex items-center gap-2 text-sm font-semibold text-[#FFD700]">
                      <span>통합 답변</span>
                      <span className="text-[10px] font-normal text-white/45">by {CATALOG_BY_ID[turn.synthesizer || ""]?.shortName || turn.synthesizer}</span>
                    </div>
                    <div className="whitespace-pre-wrap text-sm leading-6">{turn.synthesis}</div>
                  </article>
                )}
              </div>
            ))}
            ...
            {busy && (
              <div className="rounded-lg border border-[#d7b64d]/20 bg-[#07101d] px-3 py-2 text-sm text-[#d7b64d]">
          ...
          <form onSubmit={send} className="w-full min-w-0 shrink-0 border-t border-white/10 bg-[#0b1524] p-0">
            {error && <div className="border-b border-red-400/20 bg-red-950/30 px-3 py-2 text-sm text-red-200">{error}</div>}
            <div className="w-full min-w-0 rounded-t-2xl border border-[#d7b64d]/30 bg-[#07101d] p-2">
              ...
              />
              <div className="mt-1 flex h-[54px] min-h-[54px] w-full min-w-0 items-center gap-2 overflow-hidden border-t border-[#d7b64d]/35 bg-[#0b1524] px-2">
                <div className="text-xs text-white/40">Memory: {activeConversation?.turns.length || 0} turns saved</div>
                <div className="min-w-4 flex-1" />
                <button type="submit" disabled={!prompt.trim() || busy || !sessionReady} className="flex h-10 items-center gap-2 rounded-xl bg-[#d7b64d] px-6 font-semibold text-[#111827] disabled:opacity-40">
                  <Send size={17} /> Send
                </button>
              </div>
            </div>
      ...
      {warehouseOpen && (
        <div className="fixed inset-0 z-[210] flex items-start justify-center bg-black/70 px-4 pb-4 pt-[105px]" onClick={() => setWarehouseOpen(false)} role="presentation">
          <div className="flex max-h-[74vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl border border-[#d7b64d]/50 bg-[#081321] shadow-2xl" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="border-b border-white/10 px-4 py-3">
              <div className="mb-2 flex gap-1 overflow-x-auto pb-1">
                {slots.map((id, index) => (
                  <button key={`replace-${id}-${index}`} type="button" onClick={() => setReplaceSlot(index)} className={`shrink-0 rounded-md border px-2 py-1 text-[10px] ${replaceSlot === index ? "border-[#d7b64d] bg-[#d7b64d] text-[#111827]" : "border-white/10 bg-[#0b1524] text-[#c9d0da]"}`}>
              ...
              <div className="relative">
                ...
                <input value={warehouseSearch} onChange={(event) => setWarehouseSearch(event.target.value)} placeholder="AI 검색..." className="h-10 w-full rounded-lg border border-white/10 bg-[#07101d] pl-9 pr-3 text-sm outline-none" />
              </div>
            </div>
            ...
            <div className="grid min-h-0 flex-1 grid-cols-2 gap-2 overflow-y-auto p-4 sm:grid-cols-3 lg:grid-cols-4">
              {filteredWarehouse.map((ai) => {
                ...
                return (
                  <button key={ai.id} type="button" onClick={() => replaceWarehouseAI(ai.id)} className={`flex min-h-16 items-center gap-3 rounded-xl border p-3 text-left ${inSlots ? "border-[#d7b64d]/50 bg-[#d7b64d]/10" : "border-white/10 bg-[#0b1524]"}`}>
