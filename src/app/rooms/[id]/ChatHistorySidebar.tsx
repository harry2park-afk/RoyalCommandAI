      ...
      <div className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden p-2">
        <div className="space-y-2">
          {historyBoxes.map((box, index) => (
            <div
              key={`${box.ids[0]}-${index}`}
              onDoubleClick={() => void removeHistoryBox(box)}
            ...
            >
              <button type="button" onClick={() => openHistoryBox(box)} className="min-w-0 flex-1 text-left" title="한 번 클릭하면 가운데에서 보기 · 더블클릭하면 바로 삭제">
              ...
              <button type="button" onClick={(event) => { event.stopPropagation(); void removeHistoryBox(box); }} className="shrink-0 rounded-md p-1.5 text-[var(--muted)] hover:bg-red-500/10 hover:text-red-300" title="즉시 삭제"><Trash2 size={14} /></button>
            </div>
          ))}
