...
export default function CustomerAISecretary({ roomId, standalone = false }: { roomId: string; standalone?: boolean }) {
  ...
  useEffect(() => {
    ...
    const loadCalls = () => fetch(`/api/rooms/${encodeURIComponent(roomId)}/ai-secretary/calls`, { cache: "no-store" })
      .then((response) => { if (!response.ok) throw new Error("calls failed"); return response.json(); })
      .then((payload) => { if (active && Array.isArray(payload?.calls)) { setCalls(payload.calls); setCallError(""); } })
      .catch(() => { if (active) setCallError("전화 기록을 불러오지 못했습니다. 잠시 후 다시 확인해 주세요."); });
    loadCalls();
          ...
          <div className="min-h-0 flex-1 overflow-y-auto p-4">
            ...
            {tab === "대화" ? <div className="mx-auto flex h-full w-full flex-col">
              ...
              <form onSubmit={sendInstruction} className="relative mt-3 shrink-0">
                ...
                <div className="absolute bottom-3 left-3"><SecretaryVoice roomId={roomId} key={roomId} busy={busy} initialText={input} onTranscript={setInput} onMessage={submitInstruction} onActiveChange={setVoiceActive}/></div>
