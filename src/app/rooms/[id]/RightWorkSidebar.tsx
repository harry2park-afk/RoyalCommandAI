...
const DEFAULT_APPS = ["email", "instagram", "youtube", "drive", "calendar", "files", "tasks", "approval", "netflix"];
    ...
    <aside className="relative z-40 flex h-screen shrink-0 flex-col border-l border-white/10 bg-[#07111f]" style={{ width }}>
      ...
      <div className="shrink-0 border-b border-white/10 p-3">
        ...
        {searchResults.length > 0 ? (
          <div className="mt-2 max-h-48 space-y-1 overflow-y-auto rounded-xl border border-white/10 bg-[#0a1626] p-1.5 shadow-xl">
            {searchResults.map((app) => {
              ...
              return (
                <button key={app.id} type="button" onClick={() => added ? openApp(app) : addApp(app.id)} className="flex w-full items-center gap-2 rounded-lg px-2 py-2 text-left hover:bg-white/5">
                  <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-white/5 text-[var(--gold-soft)]"><AppIcon icon={app.icon} /></span>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-3">
        <div className="grid grid-cols-2 gap-2">
          {selectedApps.map((app) => (
            <div key={app.id} className="group relative rounded-2xl border border-white/10 bg-black/20 p-3 transition hover:border-[var(--gold)]/35 hover:bg-white/[0.04]">
              <button type="button" onClick={() => openApp(app)} className="w-full text-left">
                <span className="mb-3 grid h-11 w-11 place-items-center rounded-xl border border-white/10 bg-white/5 text-[var(--gold-soft)]"><AppIcon icon={app.icon} /></span>
                <span className="block truncate text-sm font-semibold">{app.title}</span>
                <span className="mt-1 block min-h-7 text-[10px] leading-3.5 text-[var(--muted)]">{app.description}</span>
              </button>
              <button type="button" onClick={() => removeApp(app.id)} className="absolute right-2 top-2 rounded-md p-1 text-white/25 opacity-0 transition hover:bg-red-500/10 hover:text-red-300 group-hover:opacity-100" title="패널에서 제거" aria-label={`${app.title} 제거`}><Trash2 size={13} /></button>
            </div>
          ))}
        </div>
      ...
      <a href="/" className="m-3 mt-0 block shrink-0 rounded-2xl border border-[var(--gold)]/25 bg-[var(--gold)]/5 p-3 text-left hover:bg-[var(--gold)]/10" aria-label="스폰서 광고 영역">
        ...
        <div className="mt-1 text-[10px] leading-4 text-[var(--muted)]">향후 광고 네트워크를 연결하면 고객이 광고를 눌러 상품·서비스를 확인할 수 있습니다.</div>
      </a>
    </aside>
  );
}
