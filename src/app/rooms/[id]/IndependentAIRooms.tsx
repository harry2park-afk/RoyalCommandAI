          ...
          <>
            <section className="relative mb-3 rounded-xl border border-white/10 bg-[#0b1524] p-2 sm:p-3">
              <form onSubmit={submitSelected} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = "copy"; }} onDrop={(event) => { event.preventDefault(); if (event.dataTransfer.files.length) addAttachments(event.dataTransfer.files); }} data-rc-composer-enhanced="true" className="relative">
                ...
                <input ref={attachmentInputRef} type="file" multiple accept="image/*,.pdf,.doc,.docx,.txt,.rtf,.odt" onChange={(event) => { if (event.target.files) addAttachments(event.target.files); event.currentTarget.value = ""; }} className="hidden" aria-label="Attach files"/>
                ...
                <div data-rc-composer-controls="true" className="absolute bottom-3 right-3 flex items-center gap-1.5">
                  ...
                  <button type="button" onClick={() => attachmentInputRef.current?.click()} className="grid h-10 w-10 place-items-center rounded-lg border border-white/15 bg-[#0b1524] text-[#d8dee8]" title="Attach files" aria-label="Attach files"><Paperclip size={17}/></button>
