...
export default function RoomPage() {
  ...
  async function send(e?: FormEvent) {
    ...
    setMessages((prev) => [
      ...prev,
      { id: `temp-user-${Date.now()}`, content: currentPrompt, authorType: "user" },
    ]);
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-7xl flex-col px-4 py-3 md:px-6">
      <header className="mb-3 flex h-12 w-full items-center gap-2 rounded-xl border border-white/10 bg-black/20 px-2">
        <Link href="/dashboard" className="shrink-0 text-xs text-[var(--muted)]">
          ← Dashboard
        </Link>

        <h1
          className="ml-2 shrink-0 whitespace-nowrap text-xl md:text-2xl"
          style={{ fontFamily: "var(--font-display), serif" }}
        >
          Royal Command AI Room
        </h1>
        ...
        <div className="ml-auto flex min-w-0 items-center gap-1.5 overflow-x-auto">
          <div className="flex shrink-0 rounded-lg border border-white/10 p-0.5">
            <button
              ...
              className={`flex h-8 items-center gap-1 rounded-md px-2.5 text-xs ${
                mode === "chat" ? "bg-[var(--gold)] text-black" : "text-[var(--muted)]"
              }`}
            >
            ...
            <button
              ...
              className={`flex h-8 items-center gap-1 rounded-md px-2.5 text-xs ${
                mode === "voice" ? "bg-[var(--gold)] text-black" : "text-[var(--muted)]"
              }`}
            >
          ...
          <button
            ...
            className={`grid h-8 w-8 shrink-0 place-items-center rounded-lg border border-[var(--line)] ${
              speakerEnabled ? "text-[var(--gold-soft)]" : "text-[var(--muted)]"
            }`}
          ...
          <select
            className="h-8 w-[108px] shrink-0 rounded-lg border border-[var(--line)] bg-[var(--bg)] px-2 text-xs text-[var(--text)] outline-none"
            value={language}
        ...
        <section className="rc-card flex min-h-[72vh] flex-col overflow-hidden">
          {mode === "voice" ? (
            <div className="border-b border-[var(--line)] p-4">
              <div className="mx-auto flex max-w-2xl flex-col items-center rounded-2xl border border-white/10 bg-black/25 p-4 text-center">
                <Bot size={40} className="text-[var(--gold-soft)]" />
                <h2 className="mt-2 text-lg font-semibold">Royal Command AI Advisor</h2>
                <button
                  type="button"
                  onClick={toggleListen}
                  className="mt-3 flex items-center gap-2 rounded-full bg-[var(--gold)] px-4 py-2 text-xs text-black"
                >
                  <Mic size={15} /> {listening ? "Listening…" : "Speak to Royal Command"}
                </button>
              </div>
            </div>
          ) : null}
          ...
          <div className="flex-1 space-y-4 overflow-y-auto p-4 md:p-6">
            ...
            {loading ? (
              <div className="text-sm text-[var(--muted)]">
                Consulting {selected.length} selected AIs…
              </div>
            ) : null}
            <div ref={bottomRef} />
          </div>
          ...
          <form onSubmit={send} className="border-t border-[var(--line)] p-2 md:p-2.5">
            <div className="mb-1 flex min-h-7 flex-nowrap items-center gap-1 overflow-x-auto rounded-lg border border-white/10 bg-black/25 px-1.5 py-1">
              ...
              <button
                type="button"
                onClick={() => setWarehouseOpen(true)}
                className="ml-auto flex h-6 shrink-0 items-center gap-1 rounded-md border border-white/10 px-1.5 text-[10px] text-[var(--muted)]"
              >
            ...
            <div className="mt-1 flex min-h-7 flex-nowrap items-center gap-0 overflow-x-auto rounded-lg border border-white/10 bg-black/20 px-0.5 py-0.5">
              <button type="button" className="rounded p-1 text-[var(--muted)]" onClick={() => wrapSelection("**")} title="Bold"><Bold size={12} /></button>
              <button type="button" className="rounded p-1 text-[var(--muted)]" onClick={() => prefixLines("• ")} title="Bullet list"><List size={12} /></button>
              <button type="button" className="rounded p-1 text-[var(--muted)]" onClick={() => prefixLines("1. ")} title="Numbered list"><ListOrdered size={12} /></button>
              <button type="button" className="rounded p-1 text-[var(--muted)]" onClick={() => wrapSelection("[", "](https://)")} title="Insert link"><Link2 size={12} /></button>
              <button type="button" className="rounded p-1 text-[var(--muted)]" onClick={copyPrompt} title="Copy"><Copy size={12} /></button>
              <button type="button" className="rounded p-1 text-[var(--muted)]" onClick={pastePrompt} title="Paste"><ClipboardPaste size={12} /></button>
              <button type="button" className="rounded p-1 text-[var(--muted)]" onClick={() => fileRef.current?.click()} title="Attach file"><Paperclip size={12} /></button>
              <button type="button" className="rounded p-1 text-[var(--muted)]" onClick={toggleListen} title="Voice input"><Mic size={12} /></button>

              <div className="ml-auto flex shrink-0 items-center gap-0.5">
                <button
                  type="submit"
                  className="flex items-center gap-0.5 rounded-full bg-[var(--gold)] px-2 py-1 text-[9px] font-bold text-black disabled:opacity-50"
                  disabled={loading || selected.length === 0}
                >
                  <Send size={10} /> Send to {selected.length} AIs
                </button>
                <button
                  type="button"
                  className="rounded-full border border-[var(--line)] px-1.5 py-1 text-[9px]"
                  onClick={() => setWarehouseOpen(true)}
                >
                  Choose AI
                </button>
      ...
      {warehouseOpen ? (
        <div
          ...
          onMouseDown={() => setWarehouseOpen(false)}
        >
          <div
            className="rc-card max-h-[88vh] w-full max-w-5xl overflow-hidden"
          ...
          >
            <div className="flex items-center justify-between border-b border-white/10 p-4">
              ...
              <button
                type="button"
                className="rounded-xl p-2 text-[var(--muted)] hover:bg-white/5"
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="mt-0.5 block text-[10px] text-[var(--muted)]">
                          {p.configured
                            ? "Connected"
                            : p.available
                              ? "Available in demo/catalog"
                              : "Catalog · connection required"}
                        </span>
                      </span>
                      {active ? (
                        <Check size={17} className="text-[var(--gold-soft)]" />
                      ) : (
                        <Plus size={17} className="text-[var(--muted)]" />
                      )}
                    </button>
                  );
                })}
