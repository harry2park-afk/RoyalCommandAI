...
describe("RCA service launch fail-closed boundaries", () => {
  ...
  it("does not create a selection or order without all required country compliance evidence", async () => {
    ...
    const selections = { upsert: vi.fn() };
    const orders = { insert: vi.fn() };
    mocks.verifyCountryServiceCompliance.mockResolvedValue({ verified: false, error: null, missingKinds: ["privacy"] });
    ...
    const response = await POST(request("compliance-blocked-service"));
