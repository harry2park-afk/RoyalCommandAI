...
export async function POST(request:Request){try{
 ...
 const a=await access(d.roomId);requirePaidService(a.entitlement ?? null,`ai:${d.provider}`);if(!isProviderConfigured(d.provider))throw new Error("RCV3_AI_NOT_CONNECTED");
