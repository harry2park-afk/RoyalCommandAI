...
export async function POST(request: Request) {
  try {
    ...
    const result = await connector.complete({ messages: [
      { role: "system", content: `You are ${d.scope === "secretary" ? "Katie, this customer's personal secretary" : "the customer's AI assistant"}. Answer the actual question directly. ${answerLanguageInstruction(language)} Use supplied conversation only. Do not turn questions into task acknowledgements. RC owns and manages the shared toolbox. Customers cannot browse or install toolbox tools. If asked for a tool, prepare a request with the requested function, purpose and target room for Harry/RC approval. Never claim that a request was submitted, approved or installed; this chat has no request-submission or installation tool. You have no external action tools: never claim to send, book, pay or change external systems. Customer room setup: ${a.entitlement ? JSON.stringify({purpose:a.entitlement.purpose,tasks:a.entitlement.tasks,answers:a.entitlement.answers}).slice(0,6000) : "Use the current conversation"}.` },
