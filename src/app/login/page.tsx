...
function LoginForm() {
  ...
  const [email, setEmail] = useState("owner@royalcommand.ai");
    ...
    <main className="mx-auto flex min-h-screen w-full max-w-md flex-col justify-center px-6 py-12">
      ...
      <div className="rc-card p-8">
        ...
        <p className="mt-2 text-sm text-[var(--muted)]">
          Local demo: owner@royalcommand.ai / password123
        </p>
        <form onSubmit={onSubmit} className="mt-8 space-y-4">
          ...
          <button className="rc-btn rc-btn-primary w-full" disabled={loading}>
            {loading ? "Signing in…" : "Enter Household"}
          </button>
        </form>
        <p className="mt-6 text-sm text-[var(--muted)]">
          New here?{" "}
          <Link href="/signup" className="text-[var(--gold-soft)]">
            Create account
          </Link>
        </p>
      </div>
