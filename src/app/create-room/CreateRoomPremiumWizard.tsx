...
function countryForLocale(locale: CreateRoomLocale) {
  return CREATE_ROOM_COUNTRIES.find((item) => item.locale === locale)?.code || "AU";
}
...
export default function CreateRoomPremiumWizard({ initialLocale, initialRoomName }: Props) {
  const startingLocale = normaliseLocale(initialLocale);
  const [step, setStep] = useState(1);
  const [request, setRequest] = useState("");
  const [industryId, setIndustryId] = useState("general");
  const [teamSize, setTeamSize] = useState("1");
  const [countryCode, setCountryCode] = useState(countryForLocale(startingLocale));
  const [locale, setLocale] = useState<CreateRoomLocale>(startingLocale);
        ...
        <header className="rounded-[28px] border border-[var(--gold)]/35 bg-black/25 p-5 shadow-[0_28px_90px_rgba(0,0,0,.35)] md:p-7">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              {initialRoomName ? <div className="mt-2 text-sm text-emerald-200">{isKorean ? "Room 이름" : "Room name"}: <strong>{initialRoomName}</strong></div> : null}
            ...
            <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-right">
              <div className="text-xs text-emerald-200">{BASIC_TRIAL_DAYS}{isKorean ? "일 무료" : " Day Trial"}</div>
              <div className="text-lg font-semibold">{money(0)} {isKorean ? "오늘" : "today"}</div>
              <div className="text-xs text-[var(--muted)]">{isKorean ? `이후 ${money(BASIC_ROOM_MONTHLY_AUD)}/월 + 선택 서비스` : `then ${money(BASIC_ROOM_MONTHLY_AUD)}/month + selected services`}</div>
            </div>
          </div>
          <div className="mt-6 grid grid-cols-5 gap-2">
            {t.steps.map((label, index) => {
              ...
              return <div key={`${label}-${number}`} className={`rounded-xl border px-2 py-2 text-center text-xs ${state}`}><span className="font-bold">{number}</span><span className="hidden sm:inline"> · {label}</span></div>;
            })}
          </div>
        </header>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_360px]">
          <section className="rounded-[28px] border border-white/10 bg-black/20 p-5 md:p-8">
            {step === 1 ? <div>
              ...
              <div className="mt-6 flex flex-wrap gap-3"><button type="button" onClick={runRecommendation} className="rounded-xl bg-[var(--gold)] px-5 py-3 font-semibold text-black">{t.aiRecommend}</button><button type="button" onClick={() => setStep(3)} className="rounded-xl border border-white/15 px-5 py-3 font-semibold">{t.direct}</button></div>
            </div> : null}
            ...
            {step === 4 ? <div>
              ...
              {initialRoomName ? <div className="mt-3 rounded-xl border border-white/10 p-3 text-sm">{isKorean ? "Room 이름" : "Room name"}: <strong>{initialRoomName}</strong></div> : null}
              ...
              <div className="mt-6 rounded-2xl border border-white/10 bg-black/10 p-5 text-sm leading-7"><div>{t.roomType}: <strong>{INDUSTRIES.find((item) => item.id === industryId)?.label}</strong></div><div>{t.team}: {teamSize} · {selectedCountry.label} · {CREATE_ROOM_LANGUAGES.find((item) => item.locale === locale)?.label}</div><div className="mt-2">{t.trial}: <strong>{BASIC_TRIAL_DAYS} {t.trialText}</strong> {money(BASIC_ROOM_MONTHLY_AUD)}/month.</div><div className="mt-2 text-[var(--muted)]">{t.basicBenefit}</div>{websiteBenefitEligible ? <div className="mt-2 font-medium text-emerald-300">{t.websiteBenefit}</div> : null}</div>
            ...
            {step === 5 ? <div><h2 className="text-2xl font-semibold">{t.step5Title}</h2><p className="mt-2 text-sm leading-6 text-[var(--muted)]">{t.step5Help}</p><label className="mt-6 flex items-start gap-3 rounded-2xl border border-white/10 bg-black/10 p-4"><input type="checkbox" checked={agreement} onChange={(event) => setAgreement(event.target.checked)} className="mt-1 h-5 w-5 accent-emerald-500" /><span className="text-sm leading-6">{t.agreement}</span></label><div className="mt-4 rounded-2xl border border-amber-400/30 bg-amber-400/5 p-4 text-sm leading-6 text-amber-100">{t.pendingIntegration}</div><button type="button" disabled={!agreement} className="mt-6 rounded-xl bg-[var(--gold)] px-5 py-3 font-semibold text-black disabled:opacity-40">{t.readyPreview}</button></div> : null}
          </section>

          <aside className="hidden h-fit rounded-[28px] border border-[var(--gold)]/30 bg-black/30 p-5 lg:sticky lg:top-6 lg:block"><div className="text-sm font-semibold text-[var(--gold-soft)]">{t.livePrice}</div><div className="mt-5 space-y-3 text-sm"><div className="flex justify-between"><span>{t.today}</span><strong>{money(0)}</strong></div><div className="flex justify-between"><span>{t.afterTrial}</span><strong>{money(monthlyKnown)}</strong></div><div className="flex justify-between"><span>{t.promotion} {PROMOTION_PERCENT}%</span><strong className="text-emerald-300">−{money(discount)}</strong></div><div className="flex justify-between"><span>{t.save}</span><strong>{money(discount)}</strong></div><div className="border-t border-white/10 pt-4"><div className="text-xs text-[var(--muted)]">{t.total}</div><div className="mt-1 text-3xl font-semibold text-[var(--gold-soft)]">{money(monthlyAfterDiscount)}</div></div>{oneTimeKnown > 0 ? <div className="flex justify-between border-t border-white/10 pt-3"><span>{t.oneTime}</span><strong>{money(oneTimeKnown)}</strong></div> : null}{hasUnconfirmed ? <div className="rounded-xl border border-amber-400/25 bg-amber-400/5 p-3 text-xs leading-5 text-amber-100">{t.priceToConfirm}</div> : null}</div><label className="mt-5 flex items-center gap-2 text-xs text-[var(--muted)]"><input type="checkbox" checked={promotion} onChange={(event) => setPromotion(event.target.checked)} className="accent-emerald-500" /> {t.promotion} {PROMOTION_PERCENT}%</label></aside>
        </div>

        <div className="fixed inset-x-3 bottom-3 z-40 rounded-2xl border border-[var(--gold)]/35 bg-black/90 p-3 shadow-2xl backdrop-blur lg:hidden"><div className="flex items-center justify-between gap-4"><div><div className="text-[10px] uppercase tracking-[0.15em] text-[var(--muted)]">{t.total}</div><div className="text-xl font-semibold text-[var(--gold-soft)]">{money(monthlyAfterDiscount)}/mo</div></div><div className="text-right text-xs"><div>{t.today}: {money(0)}</div><div className="text-emerald-300">{t.save}: {money(discount)}</div></div></div></div><div className="h-20 lg:hidden" />
      </div>
    </main>
  );
}
