    ...
    <div>
      <div className="mx-auto max-w-7xl px-4 pt-7 md:px-8 md:pt-10">
        <section className="rounded-[28px] border border-[var(--gold)]/35 bg-black/25 p-5 shadow-[0_20px_70px_rgba(0,0,0,.25)] md:p-7">
          <div className="text-[11px] font-semibold uppercase tracking-[0.22em] text-[var(--gold-soft)]">
            Royal Command · {isKorean ? "가입 고객 정보" : "Customer Profile"}
          </div>
          <h1 className="mt-2 text-2xl font-semibold md:text-3xl">
            {isKorean ? "먼저 가입 정보를 확인해 주세요" : "First, confirm your account details"}
          </h1>
          <p className="mt-2 text-sm leading-6 text-[var(--muted)]">
            {isKorean
              ? "이미 가입할 때 등록된 정보는 다시 입력하지 않습니다. 저장되지 않은 항목만 나중에 보완할 수 있습니다."
              : "Information already saved at sign-up is shown automatically. Only missing details need to be added later."}
          </p>
          ...
          <div className="mt-6 grid gap-4 lg:grid-cols-[1fr_360px]">
            <label className="block">
              <span className="text-sm font-semibold text-[var(--gold-soft)]">
                {isKorean ? "이 Room의 이름을 정해 주세요" : "Choose a name for this Room"}
              </span>
              <input
                className="rc-input mt-2"
                value={roomName}
                onChange={(event) => setRoomName(event.target.value.slice(0, 120))}
                placeholder={isKorean ? "예: 박 회계사 사무실" : "e.g. Park Accounting Office"}
              />
              <p className="mt-2 text-xs text-[var(--muted)]">
                {isKorean ? "고객이 원하는 이름으로 언제든지 정할 수 있습니다." : "You can choose the Room name you want."}
              </p>
            </label>

            <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-4">
              <div className="text-sm font-semibold text-emerald-200">
                {isKorean ? "가입 고객 무료 혜택" : "Included member benefit"}
              </div>
              <div className="mt-2 text-sm leading-6">
                {isKorean
                  ? "A$3.80 이상 가입 고객은 30일 AI 교육, 기본 시험 5회, 70점 이상 합격 시 Royal Command 전자 수료증을 무료로 받을 수 있습니다. 불합격 시 2주 추가 학습과 새 시험 5회를 합격할 때까지 무료로 반복 제공합니다."
                  : "Members on A$3.80+ plans receive the 30-day AI training program, five exam attempts per cycle, and a free Royal Command electronic Certificate of Completion after achieving 70% or higher. If not passed, an extra two-week study period and five new attempts are provided until passed."}
              </div>
            </div>
          </div>
        </section>
