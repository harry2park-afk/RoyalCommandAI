    ...
    <main className="mx-auto min-h-screen w-full max-w-7xl px-4 py-6 md:px-6 md:py-8">
      ...
      <div className="grid gap-6 xl:grid-cols-[0.8fr_1.2fr]">
        ...
        <section className="rc-card p-5 md:p-6">
          ...
          <p className="mt-2 text-sm text-[var(--muted)]">Click your business field. Its setup form opens immediately.</p>
          <div className="mt-6 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {ROOM_TEMPLATES.map((item) => (
              <button key={item.id} type="button" onClick={() => openTemplate(item.id)} className="rounded-2xl border border-white/10 bg-black/20 p-3 text-left transition hover:border-[var(--gold)] hover:bg-[var(--gold)]/10">
                <div className="flex items-center justify-between gap-2"><span className="font-medium">{item.name}</span><Check size={15} className="text-[var(--gold-soft)]" /></div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-3xl border border-[var(--gold)]/30 bg-[#0d1628] p-5 shadow-2xl md:p-7">
            <div className="flex items-start justify-between gap-4">
              <div><p className="text-xs uppercase tracking-[0.2em] text-[var(--gold-soft)]">{template.name}</p><h3 className="mt-1 text-3xl" style={{ fontFamily: "var(--font-display), serif" }}>Create Room</h3><p className="mt-2 text-sm text-[var(--muted)]">Click the choices that fit. Type only where a short answer is needed.</p></div>
                  ...
                  {field.options ? (
                    <div className="mt-3 flex flex-wrap gap-2">
                      {field.options.map((option) => {
                        ...
                        return <button key={option} type="button" onClick={() => setAnswer(field.id, option)} className={`rounded-xl border px-3 py-2 text-sm transition ${selected ? "border-[var(--gold)] bg-[var(--gold)]/15 text-[var(--gold-soft)]" : "border-white/10 bg-black/20 hover:border-white/30"}`}>{option}</button>;
                      })}
                    </div>
