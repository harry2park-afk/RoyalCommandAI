...
export default function Room({ providers, secretaryRooms, language }: { providers: { id: AIProviderId; label: string; configured:boolean }[]; secretaryRooms:{id:string;name:string}[]; language:string }) {
  ...
  return <main className={styles.root}>
    ...
    {warehouse&&<section ref={galleryRef} className={styles.gallery} role="dialog" aria-modal="true" aria-label="Room List">
      <div className={styles.galleryHeader}><div><small>ROYAL COMMAND</small><h1>Room List</h1></div><input autoFocus aria-label="Search rooms" placeholder="Search rooms…" value={roomSearch} onChange={e=>{setRoomSearch(e.target.value);setGalleryTab("Rooms");}}/><button disabled={busy} onClick={()=>setWarehouse(false)}>Close</button></div>
      <nav className={styles.galleryTabs} aria-label="Warehouse sections">{["Rooms","Connections"].map(t=><button key={t} aria-pressed={galleryTab===t} onClick={()=>setGalleryTab(t)}>{t}</button>)}</nav>
    ...
    {!state?<section className={styles.empty}><h1>Choose a room from Room List</h1></section>:<>
      ...
      {editing&&<fieldset aria-label="Button editor" disabled={busy} className={styles.editor}>
