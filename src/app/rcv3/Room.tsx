...
export default function Room({ providers, secretaryRooms, language }: { providers: { id: AIProviderId; label: string; configured:boolean }[]; secretaryRooms:{id:string;name:string}[]; language:string }) {
  ...
  const openRoom = useCallback(async (id:string) => {
    const token=++generation.current; roomRef.current=id; setRoomId(id); setState(null); setError(""); setDirty(false); setEditing(false); setSelected(null); setTurns([]); setLiveTurns([]); setText(""); setFiles([]); setShowFiles(false); setBackground("");setBatchIds([]);setCardStatus({});
  ...
  function add(capability:Button["capability"]){if(!state)return;const existing=state.design.buttons.find(b=>b.capability===capability);if(existing){setEditing(true);setSelected(existing.id);return;}const id=crypto.randomUUID();update({...state,bindings:{...state.bindings,[id]:capability},design:{...state.design,buttons:[...state.design.buttons,{id,capability,label:capability==="secretary"?"Katie":capability==="files"?"Files":"My AI",x:5,y:5,width:20,height:12,opacity:1}]}});setEditing(true);setSelected(id);}
  ...
  return <main className={styles.root}>
    ...
    {warehouse&&<section ref={galleryRef} className={styles.gallery} role="dialog" aria-modal="true" aria-label={galleryTab==="Rooms"?"Create Room":"AI List"}>
