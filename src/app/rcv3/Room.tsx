...
export default function Room({ providers, secretaryRooms, language }: { providers: { id: AIProviderId; label: string; configured:boolean }[]; secretaryRooms:{id:string;name:string}[]; language:string }) {
  ...
  async function reorderAI(id:AIProviderId,target:AIProviderId){
    ...
    order.splice(from,1);order.splice(to,0,id);busyRef.current=true;setBusy(true);setError("");
    try{await persist({...state,providerOrder:order});}catch(e){setError((e as Error).message);}finally{busyRef.current=false;setBusy(false);}
  }
  ...
  return <main className={styles.root}>
    ...
    {warehouse&&<section ref={galleryRef} className={styles.gallery} role="dialog" aria-modal="true" aria-label={galleryTab==="Rooms"?"Room List":"AI List"}>
      ...
      {galleryTab==="Rooms"?<>
        ...
        <p className={styles.galleryHint}>{creatingTemplate?"Creating your room…":roomCategory==="All"&&!roomSearch?"Choose a type, then choose your room design.":"Select a room to start using it."}</p>
      ...
      </>:state?<><h2>Select AIs</h2><div className={styles.providerGrid}>{orderedProviders.filter(p=>p.label.toLowerCase().includes(aiSearch.toLowerCase().trim())).map(p=><div key={p.id} data-ai-order={p.id} className={styles.providerChoice} style={{opacity:movingAI===p.id?.6:1}}><button type="button" className={styles.aiMove} aria-label={`Move ${p.label}`} disabled={busy||dirty} onPointerDown={e=>{if(e.button!==0)return;e.preventDefault();e.currentTarget.setPointerCapture(e.pointerId);setMovingAI(p.id);}} onPointerUp={e=>{if(!movingAI)return;const target=document.elementFromPoint(e.clientX,e.clientY)?.closest<HTMLElement>("[data-ai-order]")?.dataset.aiOrder;setMovingAI(null);if(target)void reorderAI(p.id,target as AIProviderId);}} onPointerCancel={()=>setMovingAI(null)} onLostPointerCapture={()=>setMovingAI(null)} onKeyDown={e=>{if(!["ArrowLeft","ArrowRight","ArrowUp","ArrowDown"].includes(e.key))return;e.preventDefault();const grid=e.currentTarget.closest("[data-ai-order]")!.parentElement!;const columns=getComputedStyle(grid).gridTemplateColumns.split(" ").length;const delta=e.key==="ArrowLeft"?-1:e.key==="ArrowRight"?1:e.key==="ArrowUp"?-columns:columns;const visible=orderedProviders.filter(p=>p.label.toLowerCase().includes(aiSearch.toLowerCase().trim()));const target=visible[visible.findIndex(x=>x.id===p.id)+delta];if(target)void reorderAI(p.id,target.id);}}>⠿</button><label className={styles.aiSelect}><input type="checkbox" aria-label={`Select ${p.label}`} checked={state.selectedProviders.includes(p.id)} disabled={busy||dirty||!p.configured} onChange={()=>void selectAI(p.id,!state.connectedProviders.includes(p.id))}/>{brand(p.id)}<small>{state.connectedProviders.includes(p.id)?"Connected":p.configured?"Connect":"Unavailable"}</small></label></div>)}</div><h2>Katie</h2><label>Existing secretary room <select aria-label="Katie room" value={state.secretaryRoomId??""} disabled={busy||dirty} onChange={e=>void linkKatie(e.target.value)}><option value="">Select your existing secretary room</option>{secretaryRooms.map(r=><option key={r.id} value={r.id}>{r.name} · {r.id.slice(0,8)}</option>)}</select></label></>:<p>Choose a room first.</p>}
    </section>}
    {!state?<section className={styles.empty}><h1>Choose a room from Room List</h1></section>:<>
      <section className={styles.toolbar} aria-label="Answer AIs"><button disabled={busy||dirty} onClick={()=>{setGalleryTab("Connections");setAISearch("");setWarehouse(true);}}>AI List</button>{state.selectedProviders.map(id=><button key={id} className={styles.fixedAI} title={providers.find(p=>p.id===id)?.label??id} aria-label={`Chat with ${providers.find(p=>p.id===id)?.label??id}`} disabled={busy||editing} onClick={()=>openConversation("chat")}>{brand(id)}</button>)}</section>
