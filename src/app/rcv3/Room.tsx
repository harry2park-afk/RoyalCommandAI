...
export default function Room({ providers }: { providers: { id: AIProviderId; label: string }[] }) {
  ...
  return <main className={styles.root}>
    ...
    {!state?<section className={styles.empty}><h1>나만의 AI 업무 공간</h1><p>그림과 버튼은 자유롭게, 연결은 하나로.</p><button onClick={()=>void create()} disabled={busy||dirty}>{busy?"방을 준비하고 있습니다…":"RCV3 Room6 만들기"}</button></section>:<>
      ...
      <section ref={chatPanel} className={styles.chat} aria-label={scope==="secretary"?"Katie 비서 대화":"AI 대화"}><h2>{scope==="secretary"?"Katie · 개인 비서":"AI 대화"}</h2><div className={styles.messages} aria-live="polite">{turns.map(t=><div key={t.requestId}><p className={styles.user}>{t.prompt}</p><p className={styles.answer}>{t.answer}</p><small>{(t.durationMs/1000).toFixed(2)}초</small></div>)}{busy&&<p role="status">처리 중…</p>}</div><textarea ref={messageInput} aria-label="RCV3 메시지" value={text} onChange={e=>setText(e.target.value)} placeholder="무엇을 도와드릴까요?" onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey&&!e.nativeEvent.isComposing){e.preventDefault();void send();}}}/><div className={styles.compose}>{voiceLoaded?createElement('rc-voice-control',{key:roomId,ref:(el:VoiceElement|null)=>{voiceRef.current=el;}}):<Mic/>}<button aria-label="메시지 보내기" className={styles.send} disabled={busy||!text.trim()} onClick={()=>void send()}><Send size={19}/></button></div></section>
      {showFiles&&<section ref={filesPanel} className={styles.editor}><div className={styles.toolbar}><h2>내 파일</h2><button onClick={()=>fileInput.current?.click()}>텍스트 파일 추가</button><button aria-label="파일 닫기" onClick={()=>setShowFiles(false)}><X size={18}/></button></div>{files.map(f=><button key={f.id} onClick={()=>{setText(`다음 파일을 요약해주세요.\n파일: ${f.name}\n${f.text.slice(0,11000)}`);setShowFiles(false);requestAnimationFrame(()=>{chatPanel.current?.scrollIntoView({block:"start",behavior:"instant"});messageInput.current?.focus({preventScroll:true});});}}>{f.name}</button>)}</section>}
