...
export default function Room({ providers, secretaryRooms, language }: { providers: { id: AIProviderId; label: string; configured:boolean }[]; secretaryRooms:{id:string;name:string}[]; language:string }) {
  ...
  async function create(copy=false){if(busyRef.current)return;busyRef.current=true;setBusy(true);setError("");
    try{createId.current??=crypto.randomUUID();const name=copy?`${state?.name??"RCV3"} Copy`:"RCV3 · Room6";const r=await api("rooms",{requestId:createId.current,name,...(copy?{sourceRoom:roomId}:{})});createId.current=null;const list=await api("rooms");setRooms(list.rooms);await openRoom(r.roomId);setWarehouse(false);setEditing(true);}
