...
import { getTool, placeTool, removeTool, runTool, type ToolId, type ToolActions } from "@/lib/rcv3/toolbox";
...
export default function Room({ providers, secretaryRooms, language, toolboxManager = false }: { toolboxManager?:boolean; providers: { id: AIProviderId; label: string; configured:boolean }[]; secretaryRooms:{id:string;name:string}[]; language:string }) {
  ...
  function chooseToolForPlacement(id:ToolId){
    if(!toolboxManager)throw new Error("RC approval is required to install tools.");
    if(busyRef.current||dirty||saving)throw new Error("Save or cancel current changes first.");
  ...
  function placePendingTool(event:React.MouseEvent<HTMLElement>){
    ...
    const current=liveState.current,canvas=canvasRef.current;if(!current||!canvas)return;
    const rect=canvas.getBoundingClientRect();
    const point={x:(event.clientX-rect.left)/rect.width*100,y:(event.clientY-rect.top)/rect.height*100};
    const next=placeTool(current,pendingTool,()=>crypto.randomUUID(),point);
    if(next===current){setPendingTool(null);return;}
    const oldIds=new Set(current.design.buttons.map(button=>button.id));
    const placed=next.design.buttons.find(button=>!oldIds.has(button.id));
    if(!placed)return;
  ...
  return <main className={styles.root}>
    ...
    {warehouse&&<section ref={galleryRef} className={styles.gallery} role="dialog" aria-modal="true" aria-label={galleryTitle}>
      ...
      {galleryTab==="Tools"&&toolboxManager?<Toolbox installed={state?.design.buttons.map(button=>button.capability)??[]} busy={busy||saving||dirty||!state} onInstall={chooseToolForPlacement}/>
