...
export default function Room({ providers, secretaryRooms, language }: { providers: { id: AIProviderId; label: string; configured:boolean }[]; secretaryRooms:{id:string;name:string}[]; language:string }) {
  ...
  const openRoom = useCallback(async (id:string) => {
    const token=++generation.current;editBaseline.current=null;editBackground.current=""; roomRef.current=id; setRoomId(id); setState(null); setError(""); setDirty(false); setEditing(false); setSelected(null); setTurns([]); setLiveTurns([]); setText(""); setFiles([]); setShowFiles(false); setBackground("");setBatchIds([]);setCardStatus({});
  ...
  function showGallery(tab:string){setGalleryTab(tab);setWarehouse(true);}
  function beginEdit(){editBaseline.current=liveState.current;editBackground.current=background;setEditing(true);setSelected(liveState.current?.design.buttons[0]?.id??null);}
  const toolActions:ToolActions={
    ...
    "edit-buttons":()=>beginEdit(),"upload-file":()=>{setShowFiles(true);fileInput.current?.click();},toolbox:()=>showGallery("Tools"),
  };
  ...
  return <main className={styles.root}>
    ...
    {warehouse&&<section ref={galleryRef} className={styles.gallery} role="dialog" aria-modal="true" aria-label={galleryTitle}>
      ...
      <nav className={styles.galleryTabs} aria-label="Warehouse sections">{["Tools","Rooms","Connections","MyRooms"].map(t=><button key={t} aria-pressed={galleryTab===t} onClick={()=>setGalleryTab(t)}>{({Tools:"Toolbox",Rooms:"Create Room",Connections:"AI List",MyRooms:"My Rooms"} as Record<string,string>)[t]}</button>)}</nav>
      {error&&<p role="alert" className={styles.error}>{error}</p>}
      {galleryTab==="Tools"?<Toolbox installed={state?.design.buttons.map(button=>button.capability)??[]} busy={busy||saving||dirty||!state} onInstall={addTool}/>
