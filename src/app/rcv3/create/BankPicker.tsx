...
import {australianBanks, bankNames, verifiedBankSites} from "@/lib/rcv3/australian-banks";

const defaults=["Westpac Banking Corporation","Commonwealth Bank of Australia","Australia and New Zealand Banking Group Limited"];
...
export default function BankPicker({customerNumber,language}:{customerNumber:string;language:string}) {
 ...
 function go(id:string) {if(!key)return;save([id,...recent.filter(x=>x!==id)].slice(0,6));window.location.assign(destination(id));}
 ...
 const results=useMemo(()=>australianBanks.filter(name=>name.toLowerCase().includes(query.trim().toLowerCase())||(bankNames[name]||"").toLowerCase().includes(query.trim().toLowerCase())),[query]);
 ...
 return <div>
  <p><strong>{ko?"최근 사용한 은행":"Recently used banks"}</strong></p>
  {ready&&recent.length?<div>{recent.map((id,i)=><div key={id} style={{display:"flex",gap:6,alignItems:"center",marginBottom:6}}>{button(id)}<button type="button" aria-label={`${label(id)} ${ko?"위로":"move up"}`} disabled={i===0} onClick={()=>move(i,-1)}>↑</button><button type="button" aria-label={`${label(id)} ${ko?"아래로":"move down"}`} disabled={i===recent.length-1} onClick={()=>move(i,1)}>↓</button></div>)}</div>:<div style={{display:"flex",flexWrap:"wrap",gap:6}}>{defaults.map(button)}</div>}
