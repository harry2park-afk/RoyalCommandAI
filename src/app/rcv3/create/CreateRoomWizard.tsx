      ...
      <fieldset disabled={!loaded} className={styles.form}>
        ...
        <section className={styles.verticalSection}>
          ...
          {input.purpose === "legal" && !!input.brief?.trim() && <div className={styles.practiceChoices}>
            ...
            <strong>{t("legalAiPriority")}</strong>
            <div className={styles.choices}>{purpose.fields.filter(field=>field.id==="legalAiPriority"&&field.options).flatMap(field=>field.options!.map(option=><label key={option}><input type="radio" name="legalAiPriority" checked={(input.answers.legalAiPriority?.[0]||"Highest available reasoning")===option} onChange={()=>update({...input,answers:{...input.answers,legalAiPriority:[option]}})}/>{roomFeatureLabel(option,language)}</label>))}</div>
