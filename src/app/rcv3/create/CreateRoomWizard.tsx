        ...
        <fieldset disabled={!loaded} className={styles.form}>
          ...
          {input.step === 1 && <>
            ...
            {input.secretary && <section aria-label="Secretary setup" className={styles.field}>
              ...
              <label>{creationText("secretaryPhone", "en")}<input type="tel" autoComplete="off" maxLength={40} value={input.secretarySetup.phone} onChange={e => update({ ...input, secretarySetup: { ...input.secretarySetup, phone: e.target.value } })}/></label><p><HelpText helpKey="phone"/></p>
            </section>}
          </>}
          {input.step === 2 && <>
