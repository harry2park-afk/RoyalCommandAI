...
export default function LearningRoom({language}:{language:string}){
 ...
 return <main className={styles.page}>
  ...
  <div className={styles.layout}>
   <nav aria-label="Learning list"><label>Study day<select value={day} disabled={busy} onChange={e=>{const d=Number(e.target.value);setDay(d);setNotice('');const i=lessons.findIndex(l=>l.day===d);setUnit(i);setAnswer(null);setArtifact(drafts.current[lessons[i].id]??state.work?.[lessons[i].id]?.artifact??'');setMessages([]);setMessage('');}}>{Array.from({length:30},(_,i)=><option key={i} value={i+1}>Day {i+1} · {lessons.filter(l=>l.day===i+1&&state.completed.includes(l.id)).length}/{lessons.filter(l=>l.day===i+1).length}</option>)}</select></label>{lessons.map((l,i)=>l.day===day&&<button key={l.id} disabled={busy} aria-pressed={unit===i} onClick={()=>{setUnit(i);setArtifact(drafts.current[l.id]??state.work?.[l.id]?.artifact??'');setMessages([]);setMessage('');setAnswer(null);setNotice('');}}><span>{l.id}</span><strong>{l.title}</strong>{ko&&<small>{l.koTitle}</small>}{state.completed.includes(l.id)&&<small>✓ {t("m5")}</small>}</button>)}</nav>
   <article>
  ...
  <details className={styles.exam}><summary>Course sources · checked 21 September 2026</summary><p>Current capabilities are a dated snapshot; future scenarios are not guarantees.</p><ul><li><a href="https://home.dartmouth.edu/about/artificial-intelligence-ai-coined-dartmouth" target="_blank" rel="noreferrer">Dartmouth · AI history</a></li><li><a href="https://arxiv.org/abs/1706.03762" target="_blank" rel="noreferrer">Attention Is All You Need · original Transformer paper</a></li><li><a href="https://hai.stanford.edu/ai-index/2026-ai-index-report" target="_blank" rel="noreferrer">Stanford AI Index 2026 · current capabilities and society</a></li><li><a href="https://www.nist.gov/itl/ai-risk-management-framework" target="_blank" rel="noreferrer">NIST · AI risk management</a></li></ul></details>
 </main>;
}
