    ...
    <aside className="fixed bottom-[10px] right-[10px] top-[10px] z-[260] flex w-[520px] max-w-[calc(100vw-20px)] flex-col overflow-hidden bg-[#07111f]/97 shadow-[0_22px_75px_rgba(0,0,0,.62)] backdrop-blur-md">
      <div className="relative shrink-0 border-b border-white/10 px-5 pt-3">
        <div className="absolute right-3 top-3 z-10 flex items-center gap-3">
          ...
          <button type="button" onClick={() => setOpen(false)} className="grid h-8 w-8 place-items-center rounded-full text-xl leading-none text-white/70 hover:bg-white/10">×</button>
        </div>
      ...
      <form onSubmit={submit} className="h-[70px] min-h-[70px] max-h-[70px] shrink-0 border-t border-[#d7b64d]/30 bg-[#07111f] px-3 py-1">
        ...
        <div className="flex h-[34px] items-center gap-2">
          <button type="button" onClick={() => fileRef.current?.click()} className="grid h-8 w-8 shrink-0 place-items-center rounded-full text-[#d7b64d] hover:bg-white/5" title="Attach file">📎</button>
          <button type="button" onClick={toggleMic} className={`grid h-8 w-8 shrink-0 place-items-center rounded-full ${listening ? "bg-emerald-500/20 text-emerald-300 ring-1 ring-emerald-400/70" : "text-[#d7b64d] hover:bg-white/5"}`} title="Microphone">🎤</button>
          <div className="flex h-7 min-w-0 flex-1 items-center gap-[2px] overflow-hidden px-1" aria-label="Voice waveform">
            {Array.from({ length: 36 }).map((_, index) => <span key={index} className={`w-[2px] shrink-0 rounded-full bg-[#d7b64d] ${listening ? "animate-pulse" : "opacity-45"}`} style={{ height: `${6 + ((index * 5) % 18)}px`, animationDelay: `${index * 35}ms` }} />)}
          </div>
          <button type="submit" disabled={!input.trim()} className="ml-auto grid h-8 w-8 shrink-0 place-items-center rounded-full border border-[#d7b64d] bg-[#7A0C2E] text-base text-[#ffe18a] disabled:opacity-30" title="Send">↑</button>
        </div>
      </form>
    </aside>
  );
