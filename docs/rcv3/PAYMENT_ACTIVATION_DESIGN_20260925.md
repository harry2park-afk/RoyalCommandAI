...
- Ask the bank which supported **read-only business transaction feed or API** can deliver settled credit records for this account, with transaction ID, date, amount/currency, beneficiary account and full payer reference; ask about feed availability, costs, timing, reversals and supported authorisation. Alternatively assess an accredited Australian CDR data recipient with this account's eligibility and consent. Never store online banking credentials or scrape the login page.
