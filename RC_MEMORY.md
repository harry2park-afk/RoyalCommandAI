...
1. `AGENTS.md`, `ROYAL_COMMAND_LAW.md`, 이 문서를 읽는다. 이어서 아래 기능 지도의 해당 항목만 읽는다. 매번 전체 기록을 읽지 않는다.
2. 저장소 `harry2park-afk/RoyalCommandAI`, 작업 branch `feat/independent-ai-rooms-v1-20260906`, PR **#748**을 확인한다. `git status` 및 원격 HEAD를 확인하고 다른 작업자의 변경을 덮어쓰지 않는다.
3. [기능 지도](docs/continuity/FEATURE_MAP.md)에서 기존 파일·커밋·검증을 찾는다. `rg`, `git log --all`, 관련 PR 기록으로 실제 구현을 확인한다. **보이지 않는다고 새로 만들지 않는다.** 원본을 못 찾으면 찾은 범위와 부족한 증거를 기록한다.
4. Harry의 이번 요청을 현재 상태에 연결한다. 완료된 일을 다시 실행하지 않고, 기존 기능의 보완/연결/수정으로 해결한다. 재구축이 정말 필요하면 기존 것을 쓸 수 없는 근거와 데이터 보존 방법부터 설명한다.
5. 변경 후 관련 기능을 검증하고 아래 현재 상태와 [진행 기록](docs/continuity/SESSION_LOG.md)을 갱신해 같은 branch에 저장한다. 테스트·배포·접속 확인을 구별한다. 새 문서가 생기면 기능 지도에 추가한다.

GitHub 접근이 없는 새 대화는 이 파일을 읽을 수 없다. 그 경우 저장소 연결 또는 이 문서 링크를 요청하고, 기억을 읽은 것처럼 답하지 않는다. 이 문서는 계정 간 ChatGPT 기억 공유나 모든 AI의 자동 로딩을 보장하지 않는다.
