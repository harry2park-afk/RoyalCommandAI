...
## 고객 직접 구성형 툴방 — 2026-09-21 승인

[툴방 기준 설계 v1.0](docs/rcv3/SELF_SERVICE_TOOL_ROOM_DESIGN.md): 폼/툴방에서 고객이 직접 추가, 결제 확인 후 활성화, 중지되어도 위치·설정 유지, 고객이 제거할 때만 방에서 제거. 기존 버튼·AI Helper 재사용, 고객 데이터 격리. **통합 구현은 미완료**이며 문서의 체크리스트로 진행한다. 관련 개발자는 AGENTS.md의 연결을 통해 이 기준을 확인한다.
...
## 툴박스 우선 개발 규칙 보강 — 2026-09-21

Harry 지시: 기존 모든 버튼과 작동 기능을 조사·등록하고, RC V3의 화면/방은 툴박스에서 가져와 구성한다. 없는 기능은 툴박스에 먼저 등록·검증한 후 사용한다. AGENTS.md와 SELF_SERVICE_TOOL_ROOM_DESIGN.md에 필수 규칙으로 명시했다. 이번 변경은 문서만이며 전체 툴 등록·구현 완료가 아니다.
...
- Remote application commit `6a5fcb6579df5e0f739dac73ecd2ce0795e3f791` Preview READY; 66 focused tests and build passed. Final application/control and email limitations: `docs/rcv3/TOOLBOX_APPROVAL_VERIFICATION.md`. Conflict Guard's remaining 5 warnings reviewed as precise lexical false positives; exact statement exceptions plus same-rule negative regressions replace false alarms without disabling strict mode.

### RC Office mobile artwork list — 2026-09-22
- Harry clarified total 20 video-call designs. Existing Meeting Rooms registry had 10; added the final 10 brown-desk/chair Sydney Harbour images made in mobile ChatGPT that morning (Library source timestamps 2026-09-21 20:37 UTC / 06:37 Sydney). Earlier white/black-chair trials are not added as duplicates.
- One shared `rc-office-designs.ts` feeds both Meeting Rooms (20 total) and Create Room → RC Office (10 added designs). Original mapping and checksums: `docs/rcv3/RC_OFFICE_SOURCES.json`. Existing rooms and customer data unchanged.
- FAST artwork/catalog registration work; existing toolbox `meetings` and `create-room` controls reused. No new call transport: the existing meeting page is local camera/background preview, not proof of a remote video call connection.
