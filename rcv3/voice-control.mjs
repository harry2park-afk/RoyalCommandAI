...
export class RCVoiceControl extends HTMLElement {
 ...
 async start(){
  if(this.liveDictation){
   ...
   try{
    this.#dictation=createDictation(globalThis.SpeechRecognition||globalThis.webkitSpeechRecognition,{language:this.language,onText:text=>{this.dispatchEvent(new CustomEvent('transcript',{detail:{text},bubbles:true,composed:true}));},onEnd:()=>finish('글을 확인·수정한 뒤 Send를 누르세요'),onError:message=>finish(message,true)});
    ...
    this.button.disabled=false;this.button.setAttribute('aria-pressed','true');this.button.setAttribute('aria-label','Stop microphone');this.#state('듣고 있습니다 · 말씀이 입력창에 표시됩니다');
   }catch(error){finish(error.message||'받아쓰기를 시작하지 못했습니다.',true);}
   return;
  }
