...
<div class="dock"><button type="button" aria-label="마이크 시작" aria-pressed="false"><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="9" y="2" width="6" height="13" rx="3"/><path d="M5 10v2a7 7 0 0014 0v-2M12 19v3M8 22h8"/></svg><span class="custom-label"></span></button><div class="body"><canvas aria-hidden="true"></canvas><div class="status" role="status" aria-live="polite">마이크 준비 중</div></div></div>`;
 ...
 async start(){
  ...
  try{
   ...
   recorder.onstop=async()=>{
    ...
    finally{clearTimeout(timer);if(generation===this.#generation){this.#busy=false;this.button.disabled=false;this.#abort=undefined;if(this.autoSubmit&&this.conversationActive){this.button.setAttribute('aria-label','음성 대화 종료');this.button.setAttribute('aria-pressed','true');}}}
   };
   ...
   clearTimeout(this.#setupTimer);recorder.start();this.#recordingTimer=setTimeout(()=>this.stop(),60000);this.button.disabled=false;this.button.setAttribute('aria-pressed','true');this.button.setAttribute('aria-label','녹음 끝내고 보내기');this.#state(this.autoSubmit?'듣고 있습니다':'듣고 있습니다 · 다시 누르면 전송');
  }catch(error){if(generation!==this.#generation)return;this.#cleanup();this.#state(error.name==='NotAllowedError'?'브라우저에서 마이크 사용을 허용해 주세요.':error.name==='NotFoundError'?'연결된 마이크를 찾을 수 없습니다.':'마이크를 시작하지 못했습니다. 다시 눌러주세요.',true);}
 }
 stop(){if(this.#recorder?.state==='recording'){this.button.disabled=true;this.#recorder.stop();}}
 #cleanup(abort=true){clearTimeout(this.#setupTimer);clearTimeout(this.#recordingTimer);cancelAnimationFrame(this.#frame);if(abort)this.#abort?.abort();if(this.#recorder){this.#recorder.onstop=null;this.#recorder.onerror=null;if(this.#recorder.state==='recording')this.#recorder.stop();}this.#recorder=undefined;this.#stream?.getTracks().forEach(t=>{t.onended=null;t.stop();});this.#stream=undefined;this.#context?.close().catch(()=>{});this.#context=undefined;this.#busy=false;this.button.disabled=false;this.button.setAttribute('aria-pressed','false');this.button.setAttribute('aria-label','마이크 시작');}
